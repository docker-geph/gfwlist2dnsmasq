...
Valid options are:
    ...
    --nftset4 <nftset4_name>
                IPv4 nftset name for the GfwList domains
                (If not given, nftset4 rules will not be generated.)
...
get_args(){
    ...
    if [ $OUT_TYPE = 'DNSMASQ_RULES' ]; then
        ...
        # Check nftset4
        if [ -z $NFTSET4_NAME ]; then
            WITH_NFTSET4=0
        else
            NFTSET4_TEST=$(echo $NFTSET4_NAME | grep -E '^\w+(,\w+)*$')
            if [ "$NFTSET4_TEST" != "$NFTSET4_NAME" ]; then
                _red 'Error: Please enter a valid nftset4 name.\n'
                exit 1
            else
                WITH_NFTSET4=1
            fi
        ...
        # Check nftset6
        if [ -z $NFTSET6_NAME ]; then
            WITH_NFTSET6=0
        else
            NFTSET6_TEST=$(echo $NFTSET6_NAME | grep -E '^\w+(,\w+)*$')
            if [ "$NFTSET6_TEST" != "$NFTSET6_NAME" ]; then
                _red 'Error: Please enter a valid nftset6 name.\n'
                exit 1
            else
                WITH_NFTSET6=1
            fi
